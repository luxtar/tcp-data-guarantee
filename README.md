...
테스트를 진행하기에 앞서 TCP 상태 천이도 에 대한 개념에 대해 살펴보겠다.
TCP 상태 천이도 는 OSI 7 계층 중 4 계층인 전송계층에서 TCP 프로토콜 통신 과정의 상태와 흐름을 나타낸 도표이다.
...
다음은 클라이언트와 서버의 테스트 로그 출력 결과이다. 로그 출력을 위해 netstat를 활용했다. netstat를 이용하면 실시간으로 로컬의 네트워크 상태와 소켓 버퍼의 상태를 알 수 있다.
